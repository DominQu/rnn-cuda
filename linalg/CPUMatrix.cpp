...
CPUMatrix CPUMatrix::from(const std::vector<std::vector<MatrixValType>> &other) {
...
CPUMatrix CPUMatrix::random(const MatrixSize size, const MatrixValType min, const MatrixValType max) {
...
bool CPUMatrix::operator==(const CPUMatrix& other) const {
  ...
  for (auto i = 0; i < this->getSize().total; i++) {
    if (this->cpuData[i] != other.cpuData[i]) return false;
  }
...
void CPUMatrix::show(std::ostream& outStream) const {
  for (std::size_t y = 0; y < this->getSize().height; y++) {
    for (std::size_t x = 0; x < this->getSize().width; x++) {
      ...
      if (x + 1 != this->getSize().width) outStream << ";";
    }
    outStream << "\n";
  }
}
...
void CPUMatrix::multiply(const CPUMatrix& other, CPUMatrix &result) const {
  ...
  for (std::size_t y = 0; y < result.getSize().height; y++) {
	...
	val += this->cpuData[this->coordsFrom(y, k)] * other.cpuData[other.coordsFrom(k, x)];
      }
...
void CPUMatrix::add(const CPUMatrix& other, CPUMatrix &result) const {
  for (std::size_t y = 0; y < result.getSize().height; y++) {
    for (std::size_t x = 0; x < result.getSize().width; x++) {
      result.cpuData[result.coordsFrom(y, x)] = this->cpuData[this->coordsFrom(y, x)] + other.cpuData[other.coordsFrom(y, x)];
    }
  }
}
...
void CPUMatrix::transpose(CPUMatrix &result) const {
  for (std::size_t y = 0; y < result.getSize().height; y++) {
    for (std::size_t x = 0; x < result.getSize().width; x++) {
      result.cpuData[result.coordsFrom(y, x)] = this->cpuData[this->coordsFrom(x, y)];
    }
  }
}
